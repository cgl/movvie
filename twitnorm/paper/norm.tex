...
The down side of these methods were: (1) they did not consider contextual features and (2) each of them observed that tokens have an unique normalization. However, that is not the case for the normalization task. The OOV tokens are ambiguous and without contextual information it is not possible to build models that can disambiguate transformations correctly.

Also the supervised models that are required annotated data, which is not available for normalized text~\cite{DBLP:conf/wsdm/YangL11} and difficult to create.
...
Liu et al.,2012 proposes broad coverage normalization system integrates an extended noisy channel model based on enhanced letter transformations, visual priming, string and phonetic similarity~\cite{liu2012broad}. They try to improve the performance of top n normalization candidates by integrating human perspective modeling.

Hassan and Menezes,2013 generates a normalization equivalences lexicon using Markov random walks on a contextual similarity lattice~\cite{DBLP:conf/acl/HassanM13}. One of the biggest difference between our systems are they make use a huge clean vocabulary.
