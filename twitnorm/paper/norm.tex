...
Unlike clean text, noisy text is difficult to model using standard language models. Due to noisy nature of the OOV tokens, versions of a text includes multiple different OOV tokens (ex: Table~\ref{tab:ngrams}). It is even more difficult to reach the correct version of the phrase when there is more than one OOV word in the text. A high performance normalization system should be capable of following the information flow in a noisy text as well as in clean text.
\begin{table}[tbhp]
\begin{centering}
\begin{tabular}[h]{l}
\hline
...
In the word-relatedness graph, each node is a unique set of a token and a POS tag (see Table~\ref{tab:graph}). This helps us to identify the tokens not only lexically and contextually but also (in terms of POS tags) gramatically.
